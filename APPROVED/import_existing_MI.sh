...
do
    ...
    printf "\n"
    terraform import module.$MODULE_NAME.aws_ssm_maintenance_window.AWS_MAINTENANCE_WINDOW $MAINTENANCE_WINDOW_ID 2>>terraform_import_error.txt
    TF_CMD_STATUS=$?
    if [ $TF_CMD_STATUS -eq 0 ];
    then
        printf "\n"
        echo -e "\e[0;33m SUCCESSFULLY IMPORTED THE MODULE ${MODULE_NAME} OF AWS MAINTENANCE WINDOW \e[0m"
        else
        printf "\n"
        echo -e "\e[0;31m FAILED TO IMPORT THE MODULE ${MODULE_NAME} OF AWS MAINTENANCE WINDOW \e[0m"
        printf "\n"
